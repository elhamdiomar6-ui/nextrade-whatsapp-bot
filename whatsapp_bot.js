...
const CONFIG = {
  ...
  TURKEY_PHONE: process.env.TURKEY_PHONE || '+905359685281',
...
// ============================================================
// SESSIONS UTILISATEURS (filtre B2B)
// ============================================================
// États : 'unverified' | 'verified' | 'rejected'
...
const MESSAGES = {
  ...
  b2bCheck: [
    `\u{1F44B} *Bienvenue sur NexTrade Groupe !*`,
    ``,
    `Avant de continuer, nous devons v\u00e9rifier votre profil.`,
    ``,
    `\u{1F4BC} \u00cates-vous un *acheteur professionnel* ?`,
    `(importateur, distributeur, grossiste, d\u00e9taillant B2B)`,
    ``,
    `R\u00e9pondez *OUI* ou *NON*.`
  ].join('\n'),

  b2bRetry: [
    `\u2753 Merci de r\u00e9pondre uniquement par *OUI* ou *NON*.`,
    ``,
    `\u{1F4BC} \u00cates-vous un acheteur professionnel (importateur, distributeur, grossiste) ?`
  ].join('\n'),

  b2bReject: [
    `\u{1F64F} Merci pour votre int\u00e9r\u00eat pour NexTrade Groupe.`,
    ``,
    `Notre plateforme est r\u00e9serv\u00e9e aux *professionnels B2B* (importateurs, distributeurs, grossistes).`,
    ``,
    `Si vous repr\u00e9sentez une entreprise, n'h\u00e9sitez pas \u00e0 nous \u00e9crire \u00e0 :`,
    `\u{1F4E7} contact@nextradegroupe.com`,
    ``,
    `Bonne journ\u00e9e ! \u{1F30D}`
  ].join('\n')
};
...
app.post('/webhook', async (req, res) => {
  try {
    ...
    // ── Filtre B2B ──────────────────────────────────────────
    const session = userSessions.get(from) || { state: 'unverified' };

    if (session.state === 'rejected') {
      // Utilisateur non-B2B : ne pas répondre davantage
      return res.status(200).json({ status: 'ignored', reason: 'non_b2b' });
    }

    if (session.state === 'unverified') {
      if (!session.asked) {
        // Première prise de contact : poser la question B2B
        userSessions.set(from, { state: 'unverified', asked: true });
        await sendMessage(from, MESSAGES.b2bCheck);
        return res.status(200).json({ status: 'ok', from, processed: true });
      } else {
        // Attente de la réponse OUI/NON
        const answer = body.trim().toLowerCase();
        if (['oui', 'yes', 'نعم', 'evet', 'o', 'y'].includes(answer)) {
          userSessions.set(from, { state: 'verified', asked: true });
          await sendMessage(from, MESSAGES.menu);
        } else if (['non', 'no', 'لا', 'hayır', 'n'].includes(answer)) {
          userSessions.set(from, { state: 'rejected', asked: true });
          await sendMessage(from, MESSAGES.b2bReject);
        } else {
          await sendMessage(from, MESSAGES.b2bRetry);
        }
        return res.status(200).json({ status: 'ok', from, processed: true });
      }
    }
    ...
    // Traiter le message (utilisateur vérifié B2B)
